...
public class TubeRendererInspector : Editor
{
	...
	public override void OnInspectorGUI()
	{
		...
		{
			//means the mesh doesn't exist
			EditorGUILayout.LabelField("No stats available (no mesh)");
		}
		...
		if( _colorsFoldout.boolValue )
		{
			...
			if( EditorGUI.EndChangeCheck() && newUseArray != usingArray ){
				...
				{
					if(i >= 93 && i <= 175 || i >= 340 && i <= 409)
					{
						_pointColors.GetArrayElementAtIndex( i ).colorValue = new Color(233f/255f, 231f/255f, 32f/255f, 0.0f);
					}
					else
					{
						_pointColors.GetArrayElementAtIndex( i ).colorValue = new Color(233f/255f, 231f/255f, 32f/255f, 1.0f);//Color.white;
					}
				}
				isDirty = true;
			}
